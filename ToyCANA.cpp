...
private:
    ...
    void synchronize() {
        ...
        while (!match(TOK_EOF)) {
            ...
            if (match(TOK_INT) || match(TOK_VOID) || match(TOK_IF) ||
                match(TOK_WHILE) || match(TOK_RETURN) || match(TOK_LBRACE)) {
                return;
            }
            advance();
        }
...
public:
    Parser(const vector<Token>& toks) : tokens(toks), pos(0), loopDepth(0), panicMode(false) {}
